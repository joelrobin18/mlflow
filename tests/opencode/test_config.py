...
def test_load_opencode_config_nonexistent(tmp_path):
    config_path = tmp_path / "opencode.json"
    config = load_opencode_config(config_path)
    assert config == {}


def test_load_opencode_config_valid(tmp_path):
    config_path = tmp_path / "opencode.json"
...
def test_load_opencode_config_invalid_json(tmp_path):
    config_path = tmp_path / "opencode.json"
    with open(config_path, "w") as f:
...
def test_setup_hook_config_creates_mlflow_config(tmp_path):
    config_path = tmp_path / "opencode.json"

    setup_hook_config(config_path, tracking_uri="http://localhost:5000")
    ...
    assert config["trackingUri"] == "http://localhost:5000"
