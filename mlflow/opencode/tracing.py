...
def setup_logging() -> logging.Logger:
    ...
    Creates .opencode/mlflow directory structure and configures file-based logging.
    """
...
def is_tracing_enabled() -> bool:
    """Check if MLflow Opencode tracing is enabled via environment variable."""
...
def timestamp_to_ns(timestamp: int | float | None) -> int | None:
    """Convert Opencode timestamp (milliseconds) to nanoseconds.
...
def extract_user_prompt(messages: list[dict[str, Any]]) -> str:
    ...
    Args:
        messages: List of message dictionaries from Opencode session
...
def extract_assistant_response(messages: list[dict[str, Any]]) -> str:
    """Extract the final assistant response text from messages.
...
def _set_token_usage_attribute(span, tokens: dict[str, Any]) -> None:
    ...
    Args:
        ...
        tokens: Dictionary containing token usage info from Opencode
    """
    if not tokens:
        return
...
def _create_llm_and_tool_spans(parent_span, messages: list[dict[str, Any]], start_idx: int) -> None:
    """Create LLM and tool spans for assistant responses.

    Args:
        parent_span: Parent MLflow span
        messages: List of message dictionaries from Opencode session
...
def process_session(
    ...
    """Process an Opencode session and create an MLflow trace with spans.

    Args:
        session_id: Opencode session ID
        session_info: Session metadata from Opencode
        messages: List of message dictionaries with info and parts
