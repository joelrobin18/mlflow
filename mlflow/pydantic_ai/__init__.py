...
def _is_sync_context_manager_factory(func) -> bool:
    wrapped = getattr(func, "__wrapped__", None)
    return wrapped is not None and inspect.isgeneratorfunction(wrapped)
...
def _patch_method(cls, method_name):
    ...
    elif _is_sync_context_manager_factory(method):
        _patch_streaming_method(cls, method_name, patched_stream_call)
